...
class UserRepository
{
	...
	$user->location_id = 1; //One is always online. Good enough for our purposes.
	/*
	$user->address = array_get($input, 'address');
	$user->city    = array_get($input, 'city');
	$user->state   = array_get($input, 'state');
	$user->zip     = array_get($input, 'zip');
	 */

	$user->dob           = array_get($input, 'dob');
	$user->favorite_game = array_get($input, 'favorite_game');
	$user->biography     = str_limit(array_get($input, 'biography',"Looks like ".array_get($input, 'username')." hasn't said anything about themselves! Someone should tell them to put some words here."),200);
