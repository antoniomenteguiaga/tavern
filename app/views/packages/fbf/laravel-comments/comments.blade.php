<div class="comments">
	...
	@else
		...
		<p class="comments--login--required">
			...
			<a href="{{ action('UsersController@getLogin') }}?return={{ urlencode(Request::url()) }}" class="btn">
				{{ trans('laravel-comments::messages.login_btn_text') }}
			</a>
			<a href="{{ action('UsersController@getCreate') }}?return={{ urlencode(Request::url()) }}" class="btn">
